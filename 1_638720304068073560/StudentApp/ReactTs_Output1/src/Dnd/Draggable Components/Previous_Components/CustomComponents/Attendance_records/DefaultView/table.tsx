...
import { resetAttendance_recordsToInit, setAttendance_recordsMessage } from "redux/actions";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteAttendance_records } from "services/attendance_recordsService";
import { setAttendance_recordsList } from "redux/actions";
import { getAttendance_records } from "services/attendance_recordsService";
...
export const Attendance_recordsTable: React.FC<Props> = ({
  hideShowForm,
  handleRowEdit,
  getData,
  columnDefinitions,
 ...
 const handleSearch = () => {
  ...
  const handleServerDelete = async () => {
    if (rowData) {
      const response = await deleteAttendance_records(rowData.attendance_id);
      if (response) {
        dispatch(resetAttendance_recordsToInit());
        dispatch(setAttendance_recordsMessage("Deleted Successfully"));
        getData(page, pageSize, "");
        setShowDelete(false);
      } else {
        dispatch(setAttendance_recordsMessage("Some error occurred!"));
      }
    }
  };
