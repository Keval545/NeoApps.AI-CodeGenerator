...
import { resetLeave_requestsToInit, setLeave_requestsMessage } from "redux/actions";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteLeave_requests } from "services/leave_requestsService";
import { setLeave_requestsList } from "redux/actions";
import { getLeave_requests } from "services/leave_requestsService";
...
export const Leave_requestsTable: React.FC<Props> = ({
  hideShowForm,
  handleRowEdit,
  getData,
  columnDefinitions,
 ...
 const handleSearch = () => {
  ...
  const handleServerDelete = async () => {
    if (rowData) {
      const response = await deleteLeave_requests(rowData.leave_id);
      if (response) {
        dispatch(resetLeave_requestsToInit());
        dispatch(setLeave_requestsMessage("Deleted Successfully"));
        getData(page, pageSize, "");
        setShowDelete(false);
      } else {
        dispatch(setLeave_requestsMessage("Some error occurred!"));
      }
    }
  };
