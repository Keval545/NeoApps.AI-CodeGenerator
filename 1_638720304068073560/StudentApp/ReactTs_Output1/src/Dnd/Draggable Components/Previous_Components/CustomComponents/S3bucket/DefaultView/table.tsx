...
import { resetS3bucketToInit, setS3bucketMessage } from "redux/actions";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteS3bucket } from "services/s3bucketService";
import { setS3bucketList } from "redux/actions";
import { getS3bucket } from "services/s3bucketService";
...
export const S3bucketTable: React.FC<Props> = ({
  hideShowForm,
  handleRowEdit,
  getData,
  columnDefinitions,
 ...
 const handleSearch = () => {
  ...
  const handleServerDelete = async () => {
    if (rowData) {
      const response = await deleteS3bucket(rowData.bucket_id);
      if (response) {
        dispatch(resetS3bucketToInit());
        dispatch(setS3bucketMessage("Deleted Successfully"));
        getData(page, pageSize, "");
        setShowDelete(false);
      } else {
        dispatch(setS3bucketMessage("Some error occurred!"));
      }
    }
  };
