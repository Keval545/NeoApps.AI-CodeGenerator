...
import { resetLeave_requestsToInit, setLeave_requestsList, setLeave_requestsMessage } from "redux/actions";
...
import { addLeave_requests, updateLeave_requests, getLeave_requests } from "services/leave_requestsService";
...
export const Leave_requestsForm: React.FC<Props> = ({ row, hideShowForm, getData, action,  config,}) => {
    const dispatch = useAppDispatch();
    const iValue={leave_id:'',employee_id:'',leave_start_date:format(new Date(), "yyyy-MM-dd"),leave_end_date:format(new Date(), "yyyy-MM-dd"),leave_type:'',status:'',remarks:'',isActive:'',createdBy:'',modifiedBy:'',createdAt:format(new Date(), "yyyy-MM-dd"),modifiedAt:format(new Date(), "yyyy-MM-dd")};
...
useEffect(() => {
  ...
  useEffect(() => {
    if (leave_requestsData && leave_requestsData.list && leave_requestsData.list.length === 0) {
 ...
 const closeButtonClick = (
    ...
    return (
      ...
      values.leave_id = Number(values.leave_id)
values.leave_id = Number(values.leave_id)
...
values.isActive = Number(values.isActive)
            ...
            setisSaving(true);
          if (action === 'edit') {
              const response = await updateLeave_requests(values.leave_id,values);
              if (response) {
                setisSaving(false);

                  dispatch(setLeave_requestsMessage("Updated Successfully"));
                  getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                  hideShowForm('');
              } else {
                setisSaving(false);
                  dispatch(setLeave_requestsMessage("Some error occured!"));
              }
          } else if (action === 'add') {
              const response = await addLeave_requests(values);
              if (response) {
                setisSaving(false);
                  dispatch(setLeave_requestsMessage("Added Successfully"));
                  getData(Constant.defaultPageNumber, Constant.defaultPageSize, '');
                  hideShowForm('');
              } else {
                setisSaving(false);
      ...
      validationSchema= {yup.object({
         leave_start_date: yup.string().test("validator-custom-name", function (value) { const validation = ValidationControl(value,config.leave_start_date_error_control,config.leave_start_date_error_message);if (!validation.isValid) {return this.createError({path: this.path,message: validation.errorMessage});} else {return true;}}),
leave_end_date: yup.string().test("validator-custom-name", function (value) { const validation = ValidationControl(value,config.leave_end_date_error_control,config.leave_end_date_error_message);if (!validation.isValid) {return this.createError({path: this.path,message: validation.errorMessage});} else {return true;}}),
leave_type: yup.string().test("validator-custom-name", function (value) { const validation = ValidationControl(value,config.leave_type_error_control,config.leave_type_error_message);if (!validation.isValid) {return this.createError({path: this.path,message: validation.errorMessage});} else {return true;}}),
...
{!config["leave_start_date_isHidden"] && (
                    <>
                      {config["leave_start_date_isNewline"] && < Grid xs ={ 12}
                md ={ 12}></ Grid >}
                      < Grid
                        item
                        xs = { config["leave_start_date_grid_control"] }
                        md ={ config["leave_start_date_grid_control"]}
                      >
                        < Form.Group >
                          {
                    (() =>
                    {
                    switch (config["leave_start_date_control"])
                    {
                        case "rich text editor":
                            return (
                              ...
                              <>
                                ...
                                < InputLabel >
                                      {
                                config["leave_start_date_form_new_name"] !==
                                      undefined
                                        ? config["leave_start_date_form_new_name"]
                                        : "leave_start_date"}
                                    </ InputLabel >
                            ...
                            setFieldValue(
                              ...
                              case "file":
                                return (
                                  <>
                                    <input
                                      type = { config["leave_start_date_control"] }
...
placeholder ={
    ...
    async(event) => {
        var inf =
          document.getElementById("leave_start_date");
...
disabled ={ isLoading == "leave_start_date"}
                                    >
                                      {isLoading === "leave_start_date" ? (
                                        <CircularProgress
                                          size={24}
                                          color="inherit"
                                        />
...
case "datetime":
    ...
    type = "datetime-local"
                                      ...
                                      className = "form-control"
                                      value ={
        moment(values["leave_start_date"]).format(
                                        "YYYY-MM-DD hh:mm:ss"
                                      )}
    onChange ={ handleChange}
    onBlur ={ handleBlur}
    ...
    return (
      ...
      <>
        ...
        config["leave_start_date_form_new_name"] !==
                                    ...
                                    < TextField
                                      type ={
        config["leave_start_date_control"]
          ? config["leave_start_date_control"]
          : "date"
                                      }
    name = "leave_start_date"
...
{!config["leave_end_date_isHidden"] && (
                    <>
                      {config["leave_end_date_isNewline"] && < Grid xs ={ 12}
                md ={ 12}></ Grid >}
                      < Grid
                        item
                        xs = { config["leave_end_date_grid_control"] }
                        md ={ config["leave_end_date_grid_control"]}
                      >
                        < Form.Group >
                          {
                    (() =>
                    {
                    switch (config["leave_end_date_control"])
                    {
                        case "rich text editor":
                            return (
                              ...
                              <>
                                ...
                                < InputLabel >
                                      {
                                config["leave_end_date_form_new_name"] !==
                                      undefined
                                        ? config["leave_end_date_form_new_name"]
                                        : "leave_end_date"}
                                    </ InputLabel >
                            ...
                            setFieldValue(
                              ...
                              case "file":
                                return (
                                  <>
                                    <input
                                      type = { config["leave_end_date_control"] }
                                      name="leave_end_date"
                                      key={uniquekey
    }
    id="leave_end_date"
...
placeholder ={
    ...
    async(event) => {
        var inf =
          document.getElementById("leave_end_date");
        await handleFileupload(
          inf,
          setFieldValue,
          "leave_end_date"
...
disabled ={ isLoading == "leave_end_date"}
                                    >
                                      {isLoading === "leave_end_date" ? (
                                        <CircularProgress
                                          size={24}
                                          color="inherit"
                                        />
...
case "datetime":
    ...
    type = "datetime-local"
                                      ...
                                      className = "form-control"
                                      value ={
        moment(values["leave_end_date"]).format(
                                        "YYYY-MM-DD hh:mm:ss"
                                      )}
    onChange ={ handleChange}
    onBlur ={ handleBlur}
...
{!config["leave_type_isHidden"] && (
                    <>
                      {config["leave_type_isNewline"] && < Grid xs ={ 12}
                md ={ 12}></ Grid >}
                      < Grid
                        item
                        xs = { config["leave_type_grid_control"] }
                        md ={ config["leave_type_grid_control"]}
                      >
                        < Form.Group >
                          {
                    (() =>
                    {
                    switch (config["leave_type_control"])
                    {
                        case "rich text editor":
                            return (
                              ...
                              <>
                                ...
                                < InputLabel >
                                      {
                                config["leave_type_form_new_name"] !==
                                      undefined
                                        ? config["leave_type_form_new_name"]
                                        : "leave_type"}
                                    </ InputLabel >
                            ...
                            setFieldValue(
                              ...
                              case "file":
                                return (
                                  <>
                                    <input
                                      type = { config["leave_type_control"] }
                                      name="leave_type"
                                      key={uniquekey
    }
    id="leave_type"
...
disabled ={ isLoading == "leave_type"}
                                    >
                                      {isLoading === "leave_type" ? (
                                        <CircularProgress
                                          size={24}
                                          color="inherit"
                                        />
...
case "datetime":
    ...
    type = "datetime-local"
                                      ...
                                      className = "form-control"
                                      value ={
        moment(values["leave_type"]).format(
                                        "YYYY-MM-DD hh:mm:ss"
                                      )}
    onChange ={ handleChange}
    onBlur ={ handleBlur}
