...
import { resetEmployeesToInit, setEmployeesMessage } from "redux/actions";
import { Constant } from "template/Constant";
import ConfirmationModal from "template/ConfirmationModal";
import { deleteEmployees } from "services/employeesService";
import { setEmployeesList } from "redux/actions";
...
export const EmployeesTable: React.FC<Props> = ({
 ...
 const handleSearch = () => {
  ...
  const handleServerDelete = async () => {
    if (rowData) {
      const response = await deleteEmployees(rowData.employee_id);
      if (response) {
        dispatch(resetEmployeesToInit());
        dispatch(setEmployeesMessage("Deleted Successfully"));
        getData(page, pageSize, "");
        setShowDelete(false);
      } else {
        dispatch(setEmployeesMessage("Some error occurred!"));
      }
    }
  };
