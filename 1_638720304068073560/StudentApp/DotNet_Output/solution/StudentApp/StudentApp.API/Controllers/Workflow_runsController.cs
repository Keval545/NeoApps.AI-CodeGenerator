...
namespace StudentApp.API.Controllers
{
	[Authorize]
    [ApiController]
    public class Workflow_runsController : ControllerBase
    {   ILog log4Net;
        IWorkflow_runsManager Manager;
        ValidationResult ValidationResult;
        public Workflow_runsController(IWorkflow_runsManager manager)
        {  
			log4Net = this.Log<Workflow_runsController>();
            Manager = manager;
            ValidationResult = new ValidationResult();
        }
        [CheckPermission("Workflow_runs", "Get")]
        [HttpGet]
        ...
        {try
            ...
            return Ok(Manager.GetWorkflow_runsByCreatedBy(ownername,page, itemsPerPage,orderModelList));
			}catch(Exception ex)
            {
        ...
        {try
			...
			List<OrderByModel> orderModelList = UtilityCommon.ConvertStringOrderToOrderModel(orderBy);
            string ownername = HttpContext.Items["OwnerName"] as string;
            return Ok(Manager.SearchWorkflow_runsByCreatedBy(ownername,searchKey, page, itemsPerPage,orderModelList));
			}catch(Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
            }
        ...
        [CheckPermission("Workflow_runs", "Get")]
        [HttpGet]
        [Route(APIEndpoint.DefaultRoute + "/{workflow_run_id}")]
        public ActionResult GetById(int workflow_run_id)
        {try
            {
			if (workflow_run_id<= 0) { ValidationResult.AddEmptyFieldError("workflow_run_id"); }
            ...
            string ownername = HttpContext.Items["OwnerName"] as string;
            return Ok(Manager.GetWorkflow_runsByIDByCreatedBy(ownername,workflow_run_id));
			}catch(Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
            }
        ...
        [CheckPermission("Workflow_runs", "Post")]
        [HttpPost]
        [Route(APIEndpoint.DefaultRoute)]
        public ActionResult Post(Workflow_runsModel model)
        {try
            {
                ...
                return Ok(Manager.AddWorkflow_runsWithToken(model,token));
			}catch(Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
            }
        }
        [CheckPermission("Workflow_runs", "Put")]
        [HttpPut]
        [Route(APIEndpoint.DefaultRoute + "/{workflow_run_id}")]
        public ActionResult Put(int workflow_run_id, Workflow_runsModel model)
        {try
            {
                string token = Request.Headers["Authorization"];
            ...
            return Ok(Manager.UpdateWorkflow_runsWithToken(workflow_run_id, model,token));
			}catch(Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
            }
        }
        [CheckPermission("Workflow_runs", "Delete")]
        [HttpDelete]
        [Route(APIEndpoint.DefaultRoute + "/{workflow_run_id}")]
        public ActionResult Delete(int workflow_run_id)
        {
			try
            {
                string token = Request.Headers["Authorization"];
           ...
           return Ok(Manager.DeleteWorkflow_runsWithToken(workflow_run_id,token));
		   }catch(Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
            }
        ...
        public ActionResult DeleteMultiple(List<DeleteMultipleModel> deleteParam, string andOr = "AND")
        {
            try
            {
                ...
                return Ok(Manager.DeleteMultipleWorkflow_runsWithToken(deleteParam, andOr.ToUpper(),token));
            }
            catch (Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
        ...
        public ActionResult Filter(List<FilterModel> filterColumnList, string andOr = "AND", int page = 1, int itemsPerPage = 100, string orderBy = null)
        {
            try
            {
                ...
                List<OrderByModel> orderModelList = UtilityCommon.ConvertStringOrderToOrderModel(orderBy);
                string ownername = HttpContext.Items["OwnerName"] as string;
                return Ok(Manager.FilterWorkflow_runs(ownername,filterColumnList, andOr, page, itemsPerPage, orderModelList));
            }
            catch (Exception ex)
            {
                return StatusCode(500, new APIResponse(ResponseCode.ERROR, "Exception", ex.Message));
